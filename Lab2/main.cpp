...
void ReverseTask(vector<vector<double>> &mat,
                 vector<double> &coef,
                 vector<double> &free,
                 bool &flag) {

  vector<vector<double>> newMat;
  newMat.resize(mat[0].size());
  ...
  vector<double> newCoeffs = free;
  vector<double> newFree = coef;
  mat = newMat;
  coef = newCoeffs;
  free = newFree;
  //flag=!flag;
...
std::pair<double, vector<double>> SimplexMethod(vector<double> &forcheck) {
  ...
  freedom = {3, 3, 5};
  max = true;
  ReverseTask(matrix, coeffs, freedom, max);
